...
var createGenreChart = function (container, data_source, chart_name, start_year, end_year) {
  ...
  d3.tsv(data_source, function (year_data) {
    ...
    year_data = year_data.filter(function (entry) {
      return entry.year >= start_year
        && entry.year <= end_year;
    });
    ...
    //Add second series - SpringGreen line for actual number of movies
    var c_axis = genre_chart.addColorAxis(null, "#00FF7F");
    var c_axis = genre_chart.addColorAxis("Genre", ["red","yellow","green"]);
    ...
    legend._getEntries = function () {
      ...
      //Rename the 'All' legend element to Total Movies (Total Movie - Gold )
      entries[0].stroke = '#00FF7F';
      entries[0].fill = '#00FF7F';
